...
EXECUTABLE  := gadget-plugin-engagement-ledger
...
container: ## Build container using docker
	@$(MAKE) --no-print-directory log-$@
	@docker build -t gadget-plugin-engagement-ledger:local .
...
start-db: ## Start maria db - export DB_ROOT_PASS and DB_PASS to set credentials
	@$(MAKE) --no-print-directory log-$@
	@docker run --name gadget-plugin-engagement-ledger-mariadb \
		-v ${HOME}/.gadget-plugin-engagement-ledger/db:/var/lib/mysql \
		-e MARIADB_ROOT_PASSWORD="${DB_ROOT_PASS}" \
		-e MARIADB_DATABASE=gadget_plugin_engagement_ledger_dev \
...
stop-db: ## Stop maria db
	@$(MAKE) --no-print-directory log-$@
	@docker stop gadget-plugin-engagement-ledger-mariadb
	@docker rm gadget-plugin-engagement-ledger-mariadb
